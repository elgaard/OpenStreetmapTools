...
def canonicalname(nm):
      ...
      nm3=re.sub('/v.*',' ',re.sub('den ',' ',re.sub('(cafe |i/s|v/.*| a/s|pizzeria |ristorante|restaurant|bryggeriet| house| and | pizza |the |kafe |cafe |hotel| spisehus| og grillbar| og |steakhouse | kaffebar| vinbar| conditori|produktionskøkken|traktørstedet| takeaway| take away| aps)','',nm2))).replace('/','')
      nmc=re.sub(' 2','',nm3)
#      print('xx  '+nmc)
      return nmc.replace(' ','')
